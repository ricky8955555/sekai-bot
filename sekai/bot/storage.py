...
class _PairModel(BaseModel, Generic[_T_Key, _T_Model]):
    k: _T_Key
    v: _T_Model


class _MappingData(RootModel[list[_PairModel[_T_Key, _T_Model]]], Generic[_T_Key, _T_Model]):
    pass


class MappingDataStorage(Generic[_T_Key, _T_Model]):
    path: Path
    strategy: StorageStrategy
    _mapping: dict[_T_Key, _T_Model] | None
    _model: type[_T_Model]
    ...
    @property
    def _mapping_data_type(self) -> type[_MappingData[_T_Key, _T_Model]]:
        ...
        return cast(type[_MappingData[_T_Key, _T_Model]], _MappingData.__class_getitem__(types))

    async def _load_file(self) -> dict[_T_Key, _T_Model]:
    ...
    async def _load_ref(self) -> dict[_T_Key, _T_Model]:
        if self._mapping is not None:
            return self._mapping
        mapping = await self._load_file()
        return mapping

    async def load(self) -> dict[_T_Key, _T_Model]:
        mapping = await self._load_ref()
        return dict(mapping)

    async def write(self, mapping: Mapping[_T_Key, _T_Model]) -> None:
        self._mapping = dict(mapping)
        await self._write_file()

    async def get(self, key: _T_Key, default: _T_Default = None) -> _T_Model | _T_Default:
        mapping = await self._load_ref()
        return mapping.get(key, default)

    async def update(self, key: _T_Key, value: _T_Model) -> None:
        mapping = await self._load_ref()
        mapping[key] = value
        await self._write_file()
